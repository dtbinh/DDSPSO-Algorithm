...
function[g_best_solution,bestparticle,particle,fitness]=PSO_2(MAXJ,MINJ,NP,NI)
 		 ...
 		 a1=particle(:,j-1,i);
  		 a2=particle(:,ITER(i),i);
    		 ...
    		 a4=particle(:,ITER(particlenumber),particlenumber);
      		 ...
      		 temp(J)=a1(J)+(C1.*rand).*(a2(J)-a3(J))+(C2.*rand).*(a4(J)-a5(J));
            	 temp=bind(temp,MINJ,MAXJ);
   	       	particle(:,j,i)=temp;
%		particle(:,j,i)=temp;
		fitness(i,j)=fit4(temp);
       end
